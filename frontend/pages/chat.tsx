...
// Removed ExecutionProvider import - using isolated state instead
...
// Separate component for agent sidebar - completely isolated from streaming
const AgentSidebar = memo(({ 
  ...
  // Memoized sorted agents - only for this component
  ...
  }, [agents, searchQuery, pinnedAgentIds])

  // Agent Card Component - completely isolated
  const AgentCard = memo(({ agent, isSelected, isPinned, onSelect, onTogglePin }: any) => {
    return (
      <motion.div
...
// Chat interface with completely separate state
function ChatInterface() {
  // State for UI - completely separate from streaming
  ...
  // Separate state for chat - completely isolated
  ...
  // Message management functions (memoized to prevent re-renders)
  const addMessage = useCallback((message: {type: 'user' | 'assistant', content: string}) => {
  ...
  const updateMessage = useCallback((messageId: string, updates: {content: string}) => {
  ...
  // Stable callback functions for sidebar interactions
  const handleAgentSelect = useCallback((agent: Agent) => {
    setSelectedAgent(agent)
    setConversationHistory([]) // Clear history when switching agents
  }, [])
  ...
  // Send message - with isolated state management
  async function sendMessage() {
    ...
    // Add user message using isolated state
    addMessage({
      type: 'user',
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-gray-50 to-gray-100">
      {/* Completely Isolated Agent Sidebar */}
      <AgentSidebar
        agents={agents}
          ...
          <>
            ...
            <div className="bg-white/80 backdrop-blur-xl border-b border-gray-200 px-6 py-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => handleTogglePin(selectedAgent.id)}
...
// Main component - no wrapper needed with isolated state
